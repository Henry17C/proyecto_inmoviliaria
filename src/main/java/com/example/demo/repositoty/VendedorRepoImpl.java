...
TypedQuery<Vendedor> query= this.entityManager.createQuery("SELECT v FROM Vendedor v FULL JOIN  v.clientes cl ", Vendedor.class);
	...
	@Override
	public List<Vendedor> seleccionarJoinFetch() {
		TypedQuery<Vendedor> query= this.entityManager.createQuery("SELECT v FROM Vendedor v  JOIN FETCH  v.clientes cl ", Vendedor.class);
	...
	@Override
	public List<Vendedor> seleccionarJoinFetch1(BigDecimal salarioClie) {
		TypedQuery<Vendedor> query= this.entityManager.createQuery("SELECT v FROM Vendedor v  JOIN FETCH  v.clientes cl where cl.salario= :salarioClie", Vendedor.class);
		query.setParameter("salarioClie", salarioClie);
		return query.getResultList();
